...
func (sig *Signature) Verify(hash []byte, pubKey *PublicKey) bool {
	return verify(pubKey.ToECDSA(), hash, sig.R, sig.S)
}
...
// recoverKeyFromSignature based on the same function from "btcec" package
// with usage of GOST 34.10-12 formulas
func recoverKeyFromSignature(curve *KoblitzCurve, sig *Signature, msg []byte,
	iter int, doChecks bool) (*PublicKey, error) {
	// 1.1 x = (n * i) + r
	...
	//GOST 34.10-12 calculation of Q = r^-1(sG-eR)
	//We calculate terms sR and eG separately
	invr := new(big.Int).ModInverse(sig.R, curve.Params().N)

	// first term
	invrS := new(big.Int).Mul(invr, sig.S)
	invrS.Mod(invrS, curve.Params().N)
	sGx, sGy := curve.ScalarBaseMult(invrS.Bytes())
	...
	e.Mul(e, invr)
	e.Mod(e, curve.Params().N)
	minuseRx, minuseRy := curve.ScalarMult(Rx, Ry, e.Bytes())
...
// GOST 34.10-12 elliptic curve digital signature function
func SignRFC7091(privateKey *PrivateKey, hash []byte) (*Signature, error) {
